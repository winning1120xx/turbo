...
pub struct Code {
    ...
    mappings: Vec<(usize, Option<Vc<&'static dyn GenerateSourceMap>>)>,
}
...
impl CodeBuilder {
    ...
    /// code.
    pub fn push_source(&mut self, code: &Rope, map: Option<Vc<&'static dyn GenerateSourceMap>>) {
        self.push_map(map);
        self.code += code;
    }
    ...
    fn push_map(&mut self, map: Option<Vc<&'static dyn GenerateSourceMap>>) {
