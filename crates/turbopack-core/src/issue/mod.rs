...
#[turbo_tasks::value]
struct RootIssueProcessingPath(Vc<&'static dyn Issue>);

#[turbo_tasks::value_impl]
impl IssueProcessingPath for RootIssueProcessingPath {
    #[turbo_tasks::function]
    fn shortest_path(&self, issue: Vc<&'static dyn Issue>) -> Vc<OptionIssueProcessingPathItems> {
        if self.0 == issue {
...
struct ItemIssueProcessingPath(
    ...
    AutoSet<Vc<&'static dyn IssueProcessingPath>>,
);
...
#[turbo_tasks::value(transparent)]
pub struct Issues(Vec<Vc<&'static dyn Issue>>);
...
pub struct CapturedIssues {
    issues: AutoSet<Vc<&'static dyn Issue>>,
    #[cfg(feature = "issue_path")]
...
impl CapturedIssues {
    ...
    pub fn iter(&self) -> impl Iterator<Item = Vc<&'static dyn Issue>> + '_ {
    ...
    pub fn iter_with_shortest_path(
        &self,
    ) -> impl Iterator<Item = (Vc<&'static dyn Issue>, Vc<OptionIssueProcessingPathItems>)> + '_
    {
...
impl PlainAsset {
    #[turbo_tasks::function]
    pub async fn from_asset(asset: Vc<&'static dyn Asset>) -> Result<Vc<PlainAsset>> {
