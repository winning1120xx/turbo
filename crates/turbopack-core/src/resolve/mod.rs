...
use turbo_tasks_fs::{
    ...
    FileJsonContent, FileSystemEntryType, FileSystemPath, RealPathResult,
};
...
use self::{
    ...
    parse::Request,
    pattern::{read_matches, Pattern, PatternMatch, QueryMap},
    plugin::ResolvePlugin,
};
use crate::{
    asset::{Asset, AssetOption, Assets},
    issue::{package_json::PackageJsonIssue, resolve::ResolvingIssue, IssueExt},
...
pub use exports::{ExportsValue, ResolveAliasMap};
...
pub struct ResolveResult {
    ...
    pub references: Vec<Vc<&'static dyn AssetReference>>,
}
...
impl ResolveResult {
    ...
    pub fn unresolveable_with_references(
        references: Vec<Vc<&'static dyn AssetReference>>,
    ) -> ResolveResult {
        ResolveResult {
            primary: Vec::new(),
            references,
    ...
    pub fn asset(asset: Vc<&'static dyn Asset>) -> ResolveResult {
        ResolveResult {
    ...
    pub fn assets(assets: Vec<Vc<&'static dyn Asset>>) -> ResolveResult {
        ResolveResult {
            primary: assets
                .into_iter()
    ...
    pub fn assets_with_references(
        assets: Vec<Vc<&'static dyn Asset>>,
        references: Vec<Vc<&'static dyn AssetReference>>,
    ) -> ResolveResult {
        ResolveResult {
            primary: assets
    ...
    pub fn add_reference_ref(&mut self, reference: Vc<&'static dyn AssetReference>) {
        self.references.push(reference);
    }

    pub fn get_references(&self) -> &Vec<Vc<&'static dyn AssetReference>> {
        &self.references
    }
    ...
    where
        A: Fn(Vc<&'static dyn Asset>) -> AF,
        AF: Future<Output = Result<Vc<&'static dyn Asset>>>,
        R: Fn(Vc<&'static dyn AssetReference>) -> RF,
        RF: Future<Output = Result<Vc<&'static dyn AssetReference>>>,
    {
        Ok(Self {
            primary: self
                .primary
...
async fn exists(
    ...
    refs: &mut Vec<Vc<&'static dyn AssetReference>>,
...
async fn exports_field(
    ...
    package_json: Vc<FileJsonContent>,
    field: String,
...
pub enum FindContextFileResult {
    Found(Vc<FileSystemPath>, Vec<Vc<&'static dyn AssetReference>>),
    NotFound(Vec<Vc<&'static dyn AssetReference>>),
}
...
async fn resolve_internal(
    ...
    let result = match &*request_value {
            ...
            for m in matches.iter() {
                match m {
                    ...
                    PatternMatch::Directory(_, path) => {
                        let package_json_path = path.join("package.json".to_string());
        ...
        Request::Relative {
            ...
            resolve_internal(
                context,
                Request::raw(Value::new(new_pat), *force_in_context),
                options,
            )
        }
        Request::Module {
        ...
        match resolve_into_package {
            ...
            ResolveIntoPackage::ExportsField {
                ...
                if let ExportsFieldResult::Some(exports_field) =
                    &*exports_field(package_json_path, package_json, field.clone()).await?
                {
...
async fn resolve_alias_field_result(
    result: &JsonValue,
    refs: Vec<Vc<&'static dyn AssetReference>>,
    package_path: Vc<FileSystemPath>,
