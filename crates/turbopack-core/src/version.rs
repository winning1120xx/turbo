...
pub trait VersionedContent {
    ...
    async fn update(
        ...
        // return a new `Vc<&'static dyn Version>`. In this case, we need to compare
        // version ids.
        let from_id = from.id();
        let to_id = to.id();
...
pub trait MergeableVersionedContent: VersionedContent {
    fn get_merger(self: Vc<Self>) -> Vc<&'static dyn VersionedContentMerger>;
}
...
pub trait VersionedContentMerger {
    fn merge(self: Vc<Self>, contents: Vc<VersionedContents>) -> Vc<&'static dyn VersionedContent>;
}

#[turbo_tasks::value(transparent)]
pub struct VersionedContents(Vec<Vc<&'static dyn VersionedContent>>);
